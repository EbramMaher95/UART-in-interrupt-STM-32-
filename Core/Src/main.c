...
//function definition for the UART module to be used in interrupt mode
...
void on_data_received(UART_HandleTypeDef *config_it) {
	...
	//if data received -> store the data (1 byte) into the string str
	HAL_UART_Transmit(&huart1, (uint8_t*) str, 1, 100);

	//resend the data received again
	HAL_UART_Receive_IT(&huart1, (uint8_t*) str, 1);
